...
function Conversation(options) {
	...
	var lastClient = null;
	Object.defineProperty(this, 'lastClient', {
		set: function(value) {
			lastClient = value;
		},
		enumerable: false
		...
		value: function (err) {
			...
			try
			{
				...
				if(op.client) {
					...
					if (!client) return $.__done(new Error('Couldn\'t find a registered client with the ID "' + op.client + '"'));
				}
				...
				switch(op.op) {
					...
					case 'send':
						...
						return setTimeout($.__nextop, 1);
					case 'receive':
						return client.onDataReady(function (data) {
							try {
					...
					case 'disconnect':
						...
						return setTimeout($.__nextop, 1);
					case 'drop':
						while(op.drop-- != 0 && client.received.length > 0)
							client.received.shift();
						return setTimeout($.__nextop, 1);
				}
			} catch (ex) {
				return $.__done(ex);
...
Conversation.prototype.client = function (id, initializer) {
	...
	this.lastClient = this.clients[id];
