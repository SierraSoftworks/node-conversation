	...
	Object.defineProperty(this, '__done', {
		get: function () {
			return function () {
				...
				if(args.length > 0) {
					for(var i = 0; i < fail_functions.length; i++)
						fail_functions[i].apply($, args);
				} else {
					for(var i = 0; i < success_functions.length; i++)
						success_functions[i].apply($, args);
				}

				if (!!done_function) {
					done_function.apply($, args);
...
Conversation.prototype.succeded = function(callback) {
	/// <summary>Runs the given callback if no occurs</summary>
	/// <param name="callback">A function to be called once the conversation completes</param>
	/// <returns type="Conversation"/>

	this.success_functions.push(callback);
	return this;
};
...
Conversation.prototype.failed = function(callback) {
	...
	this.fail_functions.push(callback);
	return this;
};
